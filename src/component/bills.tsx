...
import {deleteBill, downloadInvoicePdf, getAllBillsSubmitted, getPDFData, openInvoicePDF} from "@/service/bill-service";
...
const Bills = ({setShowLoading}: any) => {
    ...
    const loadData = () => {
        ...
        getAllBillsSubmitted(searchParam).then((response: any) => {
            setShowLoading(false);
            if (response && response?.status == 200 && response.data) {
        ...
        <Box height={'100%'} width={'100%'} overflow={'auto'}>
            <Box p={2} maxHeight={'90%'}>
                <Box display={'flex'} alignItems={'center'} mb={1} px={1}>
                    <Typography sx={{flexGrow: 1}} variant='h5' margin={0}>Created Bills</Typography>
                </Box>
